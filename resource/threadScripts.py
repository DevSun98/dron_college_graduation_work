...
def threadRun(DCG_ins, severSock):  # DCG: DroneController_GUI 드론조종기GUI 객체 인스턴스, Serial: 시리얼 통신 객체 인스턴스
    scriptsCallback = {'takeoff': lambda: DC_scripts.takeoff(DCG_ins),
                       ...
                       'armming': lambda : DC_scripts.armming(DCG_ins)
                       }
    ...
    if addr[0] == '127.0.0.1':
        ...
        control = 1
        scriptsCallback.get("armming", lambda: None)()  # 컴퓨터 비전과 연결되면 바로 시동이 켜짐
        DCG_ins.showEnterkey.configure(text="시동")
    ...
    while True:
        ...
        if scenario == "emergency":
            ...
            scriptsCallback.get("emergency", lambda: None)()  # Dictionary default 기능 추가
            scenario = None
