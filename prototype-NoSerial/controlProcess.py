def throttleUp(throttleScaleValue, throttleScale):
    throttleScaleValue = throttleScaleValue + 4  # 필수!! 무조건 짝수로 증가 및 감소
...
def throttleDown(throttleScaleValue, throttleScale):
    throttleScaleValue = throttleScaleValue - 4  # 필수!! 무조건 짝수로 증가 및 감소
    # serial.serialOutput(str(throttleScaleValue))
    throttleScale.set(throttleScaleValue)
...
def arm(startButton, throttleScale):
    ...
    if startButtonActivation != "#FF9664":  # 활성화x 일 때 시동On
        ...
        startButton.configure(bg="SystemButtonFace")   # 활성화o 일 때 시동Off

    return throttleScaleValue


def flightMode(flightmodeLabel, Mode):
